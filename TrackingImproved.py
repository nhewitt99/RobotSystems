...
def get_max_contour(mask, min_area=300):
    """
...
def get_color_contour(img_lab, target_colors):
    """
    ...
    for i in color_range:  # color_range from LABConfig
        if i in target_colors:
            ...
            # Update max
            contour, area = get_max_contour(color_mask)
...
def get_center(contour, img_size=(640, 480)):
    """
    ...
    img_x, img_y = getCenter(rect, roi, img_size, square_length)  # also from a * import
    world_x, world_y = convertCoordinate(img_x, img_y, img_size)  # * import

    return (world_x, world_y), box


def detect_once(img, target_colors=("red",)):
    """
    ...
    contour, color, area = get_color_contour(img_lab, target_colors)

    if contour is None:
        return None, 0, None, None
    else:
        center, box = get_center(contour)
        return center, area, color, box


def draw_detection(img, center, color, box):
    img = img.copy()
...
def try_detect(
    ...
    while time.time() - start_t <= timeout:
        ...
        # Try to detect a box
        center, area, color, box = detect_once(img)

        if show:
            frame = draw_detection(img, center, color, box)
        ...
        if len(centers) == 0:
            ...
            distance = tuple_distance(center, centers[-1])
...
if __name__ == "__main__":
    ...
    try:
        while True:
            try:
                (x, y), color = try_detect(my_camera)
